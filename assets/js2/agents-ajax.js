	...
	var agent = function() {
		...
		var datatable = $('.kt-users').KTDatatable({
			...
			data: {
				...
				source: {
					read: {
						url: window.location.origin+'/MAEM/Consultants/getagents',
			...
			,
				...
				
				{
					...
					template: function(row) {
						...
						var params ="'"+row.agent_email+"','"+row.agent_name+"',"+row.agent_id;
						var append = '' ;
						var appendSendPasswrd = '' ;
						
						
						if (row.agent_approval == 1){
							appendSendPasswrd='<button onclick="sendEmailPasswd('+params+');" class="dropdown-item"><i class="la la-link"></i> Send Email And Password </a></button>';
						...
						<div class="dropdown">\
							<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="dropdown">\
						  	...
						  	<div class="dropdown-menu dropdown-menu-right">\
								...
								<a class="dropdown-item" href="/MAEM/MemberForm/newform/' + row.agent_id + '"><i class="la la-plus"></i> Add New Member</a>\
								<button class="dropdown-item" data-toggle="modal" data-target="#sendagentlink" data-record-id2="' + row.agent_id + '" ><i class="la la-link"></i> Send New Agent Form Link</a></button>\
						  	<button class="dropdown-item" data-toggle="modal"  data-target="#sendagentsignlink" data-record-id3="' + row.agent_id + '" ><i class="la la-link"></i> Send Agent Signature Link</a></button>'+appendSendPasswrd+'\
							</div>\
						</div>\
						';
					},
...
*/
            ...
            form.ajaxSubmit({
                url: window.location.origin+'/MAEM/ConsultantsExt/sendlink',
