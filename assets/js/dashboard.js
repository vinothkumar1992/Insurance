...
var KTDashboard = function() {
    ...
    var monthlySales = function() {


var settings = {
  "url": window.location.origin+"/MAEM/dashboard/monthlysales",
  "method": "GET",
  "timeout": 0,
};
...
// Daily Sales chart.
    // Based on Chartjs plugin - http://www.chartjs.org/
    var memberexpiry = function() {
        var chartContainer = KTUtil.getByID('member_expiry');
    ...
    var agentTotal = function() {        
        if (!KTUtil.getByID('agenttotal')) {
            return;
        }
...
var settings = {
  "url": window.location.origin+"/MAEM/dashboard/agenttotal",
  "method": "GET",
  "timeout": 0,
};
...
$("#agent_total").text(data.total);
$("#gm_total").text('GROUP MANAGER :'+data.chartdata.datasets[0].data[0]);
$("#sm_total").text('SENIOR MANAGER :'+data.chartdata.datasets[0].data[1]);
$("#cs_total").text('CONSULTANTS : '+data.chartdata.datasets[0].data[2]);
        ...
        var ctx = KTUtil.getByID('agenttotal').getContext('2d');
...
var MembershipTotal = function() {        
        if (!KTUtil.getByID('Membership_Total')) {
            return;
        }
		...
		var settings = {
  "url": window.location.origin+"/MAEM/Dashboard/agerange",
  "method": "GET",
  "timeout": 0,
};
$.ajax(settings).done(function (data) {
...
$("#age_total").text(data.total);
$("#age_17_20").text('Age(17-20)'+' : '+data.chartdata.datasets[0].data[0]);
$("#age_20_30").text('Age(20-40)'+' : '+data.chartdata.datasets[0].data[1]);
$("#age_30_40").text('Age(30-40)'+' : '+data.chartdata.datasets[0].data[2]);

$("#age_40_50").text('Age(40-50)'+' : '+data.chartdata.datasets[0].data[3]);
$("#age_50_60").text('Age(50-60)'+' : '+data.chartdata.datasets[0].data[4]);
$("#age_60_70").text('Age(60-70)'+' : '+data.chartdata.datasets[0].data[5]);
$("#above_70").text('Above( 70 )'+' : '+data.chartdata.datasets[0].data[6]);
        ...
        var ctx = KTUtil.getByID('Membership_Total').getContext('2d');
        var myDoughnut = new Chart(ctx, config);
    ...
    var online_payment = function() {
        if ($('#online_payment').length == 0) {
            return;
        }

        var ctx = document.getElementById("online_payment").getContext("2d");
		...
		var settings = {
  "url": window.location.origin+"/MAEM/dashboard/onlinepayment",
  "method": "GET",
  "timeout": 0,
};
...
$.ajax(settings).done(function (data) {
		...
		$("#online_payment_total").text(data.total);
    ...
    var banking_payment = function() {
        if ($('#banking_payment').length == 0) {
            return;
        }

        var ctx = document.getElementById("banking_payment").getContext("2d");
		...
		var settings = {
  "url": window.location.origin+"/MAEM/dashboard/bankingpayment",
  "method": "GET",
  "timeout": 0,
};
...
$.ajax(settings).done(function (data) {
		...
		$("#banking_payment_total").text(data.total);
