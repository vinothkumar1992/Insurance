<?php
defined('BASEPATH') OR exit('No direct script access allowed');
?><!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
	<title align="left">UNCONFIRMED MEMBER REPORT</title>
<style>
table, th, td {
	
  border-collapse: collapse;
}


</style>

</head>
<body>
<table style="width:100%">
  <tr>
    <th></th>
    <th></th>
 
  </tr>
  <tr>
    <td align="right" width=110px></td>
    <td  align="center">
	<img src="data:image/png;base64," width="80"/>
	<h4>
	MYANGKASA EMAS MEDICARE SDN BHD</h2>
	<div>
	(1281203-K)
	</div>
	</td>
   
  </tr>
  
  <tr>
    <td></td>
    <td  align="center">
	<h4>
	UNCONFIRMED MEMBER REPORT</h2>
	</td>
   
  </tr>
 
</table>

<br>

<TABLE BORDER=1 CELLSPACING=10 CELLPADDING=5px align="left" style="margin-left:1px" >

<TR HEIGHT=19 >
<TH   BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>DATE</FONT></TD>
<TH   BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM NAME</FONT></TD>
<TH   BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM ORDERID</FONT></TD>
<TH  BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM NRIC</FONT></TD>
<TH BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM EMAIL</FONT></TD>
<TH   BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM PHONE NO</FONT></TD>
<TH    BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM PAYMENT TYPE</FONT></TD>
<TH   BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM PAYMENT STATUS</FONT></TD>
<TH    BGCOLOR=#FFFFFF ><FONT style=FONT-SIZE:11pt FACE="Calibri" COLOR=#000000>FORM MEMBER APPROVAL</FONT></TD>


</TR>
 <?php  foreach($new_row as $row):?> 
  <tr>
                <td><?php
$newDate = date("d-m-Y", strtotime($row['form_cdate']));  
 echo $newDate; ?></td>
                <td><?=$row['form_name'];?></td>
				  <td><?=$row['form_orderid'];?></td>
                <td><?=$row['form_nric'];?></td>
				  <td><?=$row['form_email'];?></td>
                <td><?=$row['form_phoneno'];?></td>
				  <td><?=$row['form_payment_type'];?></td>
                <td><?=$row['form_payment_status'];?></td>
				  <td><?=$row['form_member_approval'];?></td>
               
            </tr>
  <?php endforeach;?>

</TABLE>


</body>
</html>